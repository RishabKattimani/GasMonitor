...
function getGasPrice(rpcUrl) {
    try {
        ...
        return axios
            .post(rpcUrl, payload).then(response => {
                const gasPrice = parseInt(response.data.result, 16)
                return gasPrice;
            });
        } catch (error) {
            console.log(`An error occurred while retrieving the gas price for ${rpcUrl}: ${error.message}`);
...
function logGasPriceInDatabase(gasPrices) {
    ...
    connection.connect(error => {
        ...
        const query = 'INSERT INTO gas_prices (eth, polygon, pulsechain, fusion, fantom) VALUES (?, ?, ?, ?, ?)';
...
if (isMainThread) {
    async function calculateGasPrices() {
        ...
        for (const rpcUrl of rpcUrls) {
            try {
                ...
                console.error(`An error occurred while calculating gas prices for ${rpcUrl}: ${error.message}`);
              }
        }
    ...
    const rpcUrl = workerData

    getGasPrice(rpcUrl)
        .then(gasPrice => {
            parentPort.postMessage(gasPrice);
        })
        .catch(error => {
            console.error(error);
        });
